...
gwas.results.pr <- ggplot(pyseer.results.filtered.plot, aes(x = precision, y = recall, label = label, colour = funct)) +
  ...
  geom_text_repel(size = 2, box.padding = 0.5, max.overlaps = 20, colour = "black") +
  stat_cor(method = "pearson", label.x = 0.8, label.y = 0.95, size = 5, colour = "black") +
  scale_color_brewer(palette = "Paired") +
  scale_x_continuous(limits = c(0.25, .9)) +
  scale_y_continuous(limits = c(0.25, .9)) +
  theme_bw(base_size = 14) +
  labs(
    x = "Precision",
    y = "Recall",
...
ggsave("Figure3A.pdf", gwas.results, width = 9, height = 5)
